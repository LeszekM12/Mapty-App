...
export function wmoToWeather(code) {
    // Exact match first
    if (WMO_MAP[String(code)])
        return WMO_MAP[String(code)];
    // Range fallbacks
    if (code <= 2)
        return WMO_MAP['2'];
    if (code <= 3)
...
export function formatSunset(isoStr) {
    if (!isoStr)
        return '—';
    const d = new Date(isoStr);
...
function renderWidget(data) {
    ...
    document.querySelectorAll('.weather-widget__sun')
        .forEach(el => { el.textContent = `🌅 ${data.sunset}`; });
}
// ── Public API ────────────────────────────────────────────────────────────────
const DEFAULT_COORDS = [54.09, 18.79]; // Tczew area
...
export function initWeatherWidget(coords = DEFAULT_COORDS) {
    let intervalId = null;

// ─── OFFLINE DETECTOR + SKELETON LOADER ─────────────────────────────────────
import { NetStateType } from '../types/index.js';
import { qidSafe } from '../utils/dom.js';

// ── Shared network state ──────────────────────────────────────────────────────

export const NetState: NetStateType = {
  isOffline:  false,
  mapReady:   false,
  retryCount: 0,
  timeoutId:  null,
...
export function startMapTimeout(): void {
  if (NetState.timeoutId) clearTimeout(NetState.timeoutId);
...
export function exitOfflineMode(onReconnect: () => void): void {
  if (!NetState.isOffline) return;
...
export function initOnlineDetector(onReconnect: () => void): void {
...
export function initRetryBtn(onRetry: (pos: GeolocationPosition) => void): void {
  qidSafe('btnRetry')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (NetState.retryCount >= 2) {
        ...
        else qidSafe('skeletonMsg')?.classList.remove('hidden');
      } else {
        qidSafe('skeletonMsg')?.classList.remove('hidden');
      }
    }
